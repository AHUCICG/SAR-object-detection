...
def parse_xml(args):
    ...
    for obj in objects.findall('object'):
        na = obj.find('possibleresult')
        name = na.find("name").text
        label = label_ids[name]
        # difficult = int(obj.find('difficult').text)
        difficult = 0

        boxs = obj.find('points')
        bnd_box = boxs.findall("point")
        bnd_box = [i.text for i in bnd_box]
        x=[]
        y=[]
        for i in bnd_box:
...
def cvt_annotations(devkit_path, years, split, out_file):
    ...
    for year in years:
        ...
        # xml_paths = [
        #     osp.join(devkit_path, f'/home/ahu-xiarunfan/AIR-SARShip-2.0-xml/AIR-SARShip-2.0-xml')

            for img_name in img_names
        ]
        img_paths = [
            osp.join(devkit_path, f'VOC{year}/JPEGImages/{img_name}.tif') for img_name in img_names
        # img_paths = [
        #     f'/home/ahu-xiarunfan/AIR-SARShip-2.0-data/AIR-SARShip-2.0-data' for img_name in img_names
        ]
...
def parse_args():
    ...
    parser.add_argument('--devkit_path', default='/media/ahu-xiarunfan/软件/trainData/trainData',help='pascal voc devkit path')
    parser.add_argument('-o', '--out-dir', default='/home/ahu-xiarunfan/下载/Swin-Transformer-Object-Detection-master/data/zkb',help='output path')
    parser.add_argument(
        '--out-format',
        default='coco',
        choices=('pkl', 'coco'),
